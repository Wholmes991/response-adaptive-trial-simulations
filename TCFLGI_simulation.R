#Function: TCFLGIsimulation (and FLGIallocationprobability)
#Purpose: to simulate a clinical trial using our own TCFLGI design decribed in
...
#tune1 = tuning parameter relating to the control allocation using the TP method
...
TCFLGIsimulation <- function(K,J,b,fisherco,p,sims,prior,mc,tune1){
      ...
      for (i in 1:sims){
            ...
            for (j in 3:3){
                  ...
                  probrest <- FLGIallocationprobability(posterior[2:K,], b, mc)
                  probcontrol <- (1/(K-1))*exp(max(n[1,2:K]) - n[1,1])^(tune1*(j/J)) #Different definition of K to Villar
                  prob <- c(probcontrol,probrest)
                  prob <- prob/sum(prob)
            ...
            for (j in 4:J){
                  ...
                  #calculate probabilities for rest of block
                  probrest <- FLGIallocationprobability(posterior[2:K,], b, mc)
                  probcontrol <- (1/(K-1))*exp(max(colSums(n[1:(j-2),2:K])) - sum(n[1:(j-2),1]))^(tune1*(j/J)) #Different definition of K to Villar
                  prob <- c(probcontrol,probrest)
                  prob <- prob/sum(prob)
